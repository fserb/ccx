...
// Not the panel's colors; the menu bar's background is the desktop. Luminance .58 and
// .26, both under the gold's .69. free is flat rather than translucent so it survives a
// light bar, where a 45% white would vanish.
...
// What is centered is the BLOCK OF CELLS THE DOTS USE, not the whole grid and not each
// row on its own; centering rows individually reads as a triangle rather than a grid with
// a hole. See `## The icon` in CLAUDE.md for the rest of the argument and the numbers.
...
// Paint the ring between `inner` and `outer` radius, source-over, into an RGBA buffer.
//
// A disc is this with inner 0, which is what keeps one function drawing both shapes.
// Coverage is an SSxSS grid of samples per pixel rather than an analytic area: at these
// sizes the difference is under a level of alpha and the sampler is four lines. Only the
// dot's bounding box is walked, so the cost is the ink and not the canvas.
function stamp(rgba, px, cx, cy, outer, inner, color) {
  ...
  for (let y = y0; y < y1; y++) {
    for (let x = x0; x < x1; x++) {
      ...
      if (!hit) continue;
      // straight alpha and not premultiplied, which is what a PNG carries. The dots never
      // touch, so the blend only ever runs over transparency; it is here so that a future
      // overlap is a dimmer dot rather than a hole punched in the one underneath
...
// One PNG chunk: length, type, data, and a CRC over the type and the data together.
...
// `w` by `h` straight-alpha RGBA bytes as a PNG: 8-bit, one IHDR, one IDAT, one IEND.
// IDAT wants a zlib stream, which is what "deflate" produces; "deflate-raw" omits the
// header and would make the file unreadable rather than throw.
